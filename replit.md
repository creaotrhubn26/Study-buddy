...
- **1.1 Data Analysis, Key Performance Indicators (KPIs), and Version Control**: why gut instinct is insufficient, KPIs as lens and yardstick, lagging vs leading as the two main types, quantitative KPIs split into financial and non-financial metrics with the six named metrics, data collection sources for quantitative KPIs, qualitative KPIs (customer satisfaction and employee engagement) and their manual collection methods, contextualisation, strategic alignment, heuristics, KPI types and industry applications, six real-world case studies, steps for establishing a KPI dashboard, version control for data integrity and collaboration, and **Activity 1.1.1** (the lesson task) with its formula list, method, and a worked example
